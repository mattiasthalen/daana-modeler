# USS Fixture: Columnar Dates, Historical (Type 2)
...
-- ============================================================
-- UNION ALL: Combine all entities into the bridge (columnar — no unpivot)
-- ============================================================
SELECT
...
LEFT JOIN uss.sales_order_detail p_self
    ON sodv.SALES_ORDER_DETAIL_KEY = p_self.SALES_ORDER_DETAIL_KEY
    AND sodv.valid_from >= p_self.effective_from
    AND sodv.valid_from < p_self.effective_to
...
LEFT JOIN uss.sales_order p_self
    ON sov.SALES_ORDER_KEY = p_self.SALES_ORDER_KEY
    AND sov.valid_from >= p_self.effective_from
    AND sov.valid_from < p_self.effective_to
