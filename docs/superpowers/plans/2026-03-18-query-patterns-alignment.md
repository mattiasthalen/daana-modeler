# Query Patterns Alignment Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Fix invalid SQL generation in `/daana-query` by extracting detailed query patterns from teach_claude_focal into a new reference file and updating the skill flow with hard gates.

**Architecture:** Create `query-patterns.md` as a colocated reference alongside the existing dialect/connection/framework files. Update SKILL.md to reference it and add time-dimension hard gates. All SQL templates come from teach_claude_focal's `agent_workflow.md`, adapted to the plugin's lowercase convention.

**Tech Stack:** Markdown skill files (no runtime code)

**Worktree:** `.worktrees/feat/query-patterns-alignment` on branch `feat/query-patterns-alignment`

**Important:** All file paths below are relative to the worktree root at `/home/mattiasthalen/repos/daana-modeler/.worktrees/feat/query-patterns-alignment`. Git commands must run from that directory and use `dangerouslyDisableSandbox: true` due to worktree `.git` file permissions.

---

### Task 1: Create `query-patterns.md` — ROW_ST rules and Latest pattern

**Files:**
- Create: `plugin/skills/query/query-patterns.md`

**Step 1: Create the file with ROW_ST rules and Pattern 1 (Latest)**

Write `plugin/skills/query/query-patterns.md` with this exact content:
...
Run: `wc -l plugin/skills/query/query-patterns.md`
...
git commit -m "feat: add query-patterns.md with ROW_ST rules and Latest pattern"
```

---

### Task 2: Add Temporal Alignment pattern to `query-patterns.md`

**Files:**
- Modify: `plugin/skills/query/query-patterns.md` (append after Pattern 1)

**Step 1: Append Pattern 2 — Temporal Alignment**

Append the following content to the end of `plugin/skills/query/query-patterns.md`:
...
```bash
git add plugin/skills/query/query-patterns.md
git commit -m "feat: add Temporal Alignment pattern (5-stage CTE) to query-patterns.md"
```

---

### Task 3: Add Relationship queries, Bootstrap fallback, Lineage, and Worked example

**Files:**
- Modify: `plugin/skills/query/query-patterns.md` (append after Pattern 2)

**Step 1: Append remaining patterns**
...
```bash
git add plugin/skills/query/query-patterns.md
git commit -m "feat: add relationship patterns, bootstrap fallback, lineage, and worked example"
```

---

### Task 4: Update SKILL.md — Add critical rules and reference to query-patterns.md

**Files:**
- Modify: `plugin/skills/query/SKILL.md:14-18` (Scope section)
- Modify: `plugin/skills/query/SKILL.md:148-149` (Phase 3 opening)

**Step 1: Add critical rules to Scope section**

In `plugin/skills/query/SKILL.md`, find the Scope section (lines 14-18) and add three new rules after the existing five:
...
**Step 2: Add query-patterns.md reference at Phase 3 opening**

In `plugin/skills/query/SKILL.md`, find the line `## Phase 3: Query Loop` (line 148) and add after it:
...
Run: `grep -n "Never assume physical" plugin/skills/query/SKILL.md && grep -n "query-patterns.md" plugin/skills/query/SKILL.md`
...
git commit -m "feat: add critical rules and query-patterns.md reference to SKILL.md"
```

---

### Task 5: Update SKILL.md — Add time-dimension hard gate

**Files:**
- Modify: `plugin/skills/query/SKILL.md` (Phase 3, after matching section, before query patterns section)

**Step 1: Add the time-dimension hard gate**

In `plugin/skills/query/SKILL.md`, find the section `### Query patterns` (around line 160 after previous edits) and insert the following **before** it:
...
**Step 2: Verify the hard gate is in place**

Run: `grep -n "HARD-GATE" plugin/skills/query/SKILL.md`
Expected: Should show 4 HARD-GATE markers (profile confirmation, bootstrap consent, time dimension, execution consent)

**Step 3: Commit**

```bash
git add plugin/skills/query/SKILL.md
git commit -m "feat: add time-dimension hard gate with don't-ask-again option"
```

---

### Task 6: Update SKILL.md — Remove inline patterns, replace with reference

**Files:**
- Modify: `plugin/skills/query/SKILL.md` (Phase 3, query patterns and ROW_ST sections)

**Step 1: Replace inline patterns with reference**

In `plugin/skills/query/SKILL.md`, find the section starting with `### Query patterns` and ending just before `### Lineage tracing`. This includes:
- `### Query patterns` (with Patterns 1-4 and Relationship queries subsections)
- `### ROW_ST filtering rules`
...
**Step 2: Verify the inline patterns are gone**

Run: `grep -c "Pattern 1\|Pattern 2\|Pattern 3\|Pattern 4\|UNION ALL" plugin/skills/query/SKILL.md`
Expected: 0
...
```bash
git add plugin/skills/query/SKILL.md
git commit -m "refactor: replace inline query patterns with reference to query-patterns.md"
```
...
Update the `version` field (e.g., `"0.5.0"` → `"0.6.0"`, or whatever the current version is — bump minor since this is a feature).
...
**Step 2: Verify the branch is clean and all commits are pushed**

```bash
git status
git log --oneline origin/feat/query-patterns-alignment..HEAD
```

Expected: Clean working tree, 0 unpushed commits.
